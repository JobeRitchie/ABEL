...
class FeaturePrepService:
    ...
    def prepare(
        ...
        *,
        ...
        cached_pose = self.cached_pose_sessions(project_root) if config.reuse_cached else set()
        cached_ctx = self.cached_context_sessions(project_root) if config.reuse_cached else set()
