...
CHANGELOG: list[tuple[str, str, list[str]]] = [
    ...
    ("0.13.0", "August 31, 2026", [
        ...
        "counts, durations, latencies) as two independent views; an ROI scope control "
        "joins them, so \"grooming, but only in the open arm\" is a single selection "
        "rather than a manual cross-reference. Choosing a zone rewrites the underlying "
        "bout table, which is what makes the scope reach the summary table, the "
        "statistics, every chart style, the ethograms and the exports at once instead "
        "of only the chart on screen. Because a bout can straddle the boundary, the "
        "attribution rule is exposed rather than hard-coded: split the bout at the "
        "boundary (exact durations, but a bout crossing twice counts twice), keep it "
        "if it started in the zone (counts and latencies stay comparable to the "
        "unscoped analysis), or keep it if most of its frames were in the zone. "
        "Boundary flicker shorter than a threshold is debounced into the surrounding "
        "run so tracking jitter cannot shred one bout into fragments, and bout frames "
        "are un-rebased to raw video frames before meeting the pose-derived ROI mask "
        "— the two live in different frame spaces whenever a session was prechopped. "
        "ROI-occupancy and distance pseudo-behaviors are deliberately left unscoped: "
        "time in ROI 1 restricted to ROI 1 is a tautology.",
        "Motif HMM states get a clock back. The HMM is fitted over the bout index, so "
        "its decoded states carried no time — you could see which states existed but "
        "not when an animal was in them. Runs of consecutive same-state bouts are now "
        "mapped back to session time and drawn two new ways: a per-subject state "
        "ethogram, and latency from assay start to the first qualifying entry into "
        "each state, where a dwell threshold suppresses one-bout touches and animals "
        "that never entered are drawn as open markers and excluded from the group "
        "mean rather than folded in at the session length, which would report a "
        "number the data never measured. The same state bouts export two ways: an "
        "ethogram CSV as a frame-by-subject matrix of state numbers, and a "
        "TRACY-readable state boutframes workbook. That workbook is written by the "
        "same writer as the behavior boutframes export, so a state column and a "
        "behavior column land on a photometry signal identically and the subject IDs "
        "match the ABELposition.csv files.",
        "State occupancy can be measured from the posterior instead of the Viterbi "
        "path. Viterbi occupancy is a thresholded readout: entering a state costs two "
        "transition penalties, so the most likely path will not pay them for one or "
        "two bouts and a subject who did perform a rare behavior reports exactly "
        "0.000 occupancy in that behavior's state — a floor effect that looks like "
        "data. Expected occupancy from the forward-backward posteriors reports the "
        "fraction it actually was in that state, and the choice is a control with the "
        "trade-off in its tooltip and the formula in the Methods tab. Two interval "
        "sets address the same gap for signal alignment: the top-N bouts most "
        "consistent with each state (the same rule applied to every subject, so a "
        "group difference is never confounded with which rule each subject got), and "
        "the unclaimed stretches no state bout covers, which serve as the null "
        "baseline a within-subject comparison needs.",
        "HMM fits persist between sessions. A completed fit is written to the project "
        "and restored on load, keyed on a fingerprint of everything it depends on — "
        "the behaviors in the fit, the sessions and their bout structure, the "
        "settings and the session grouping — so a restored result that no longer "
        "matches the current data is labelled STALE rather than silently presented as "
        "current. Failing to write the cache logs and is swallowed; it never breaks "
        "the analysis that just finished.",
        "New validation analysis: one UMAP holding every labeled clip from every "
        "project. The rest of the suite asks how well behavior X in project P is "
        "detected; this asks the map question — laid side by side in one feature "
        "space, where do all the assays' behaviors sit relative to each other? Three "
        "things keep it honest, each a setting rather than a hidden default. The "
        "feature space is explicit: across the eight manuscript projects the column "
        "union is 5172 and the intersection 1282, and the default embeds on the "
        "intersection so a cluster cannot be an artifact of a column only one project "
        "has. Project identity is treated as the confound it is and measured every "
        "run — silhouette and kNN purity for behavior and for project, in the feature "
        "space and in the embedding, with the chance floor for project purity stated "
        "— so the figure is never read without the number saying whether it is a "
        "behavior map or a project map. And behaviors stay assay-scoped by default, "
        "matching the rest of the suite: an EPM Rear and an open-field Rear are not "
        "pooled unless that is switched on deliberately.",
        "The map's rendering is split from its computation, because the reducer takes "
        "minutes and the figure takes a second. Coordinates are saved to "
        "embedding.parquet and a Re-render Figure button redraws from them, so "
        "palette, hulls, faceting and label placement can be tuned repeatedly without "
        "re-running anything; an embedding from an earlier run can be loaded and "
        "restyled. Cluster labels are the part with real machinery behind them, since "
        "a label dropped on its own centroid is unreadable on a dense map — each is "
        "anchored (centroid, medoid, or density peak), pushed off the cluster by a "
        "distance measured as a fraction of the axis span so it means the same thing "
        "at any zoom, then repelled from its neighbours on a leash that stops it "
        "wandering to a different cluster, with a leader line back to the anchor. A "
        "perimeter mode packs every label out to the figure rim in bearing order so "
        "leader lines never cross, which is the only arrangement that stays readable "
        "once thirty-odd clusters pile into one region.",
        "The calibration test fixture was sized so the cross-validation stage "
        "actually runs. With the larger fixture the estimator skipped CV against its "
        "time budget, which both cost a minute of suite time and left that branch "
        "untested.",
    ]),
    ...
    ("0.12.0", "August 27, 2026", [
        ...
        "over a thousand to reach, so every candidate model was stopped mid-fit — "
        "and because truncation bites harder the more states a model has, the "
        "information criteria being compared were not comparable. Restarts were also "
        "unseeded, so the same data could report a different state count on a second "
        "run, and hmmlearn's own convergence flag reads True whenever EM merely "
        "exhausts its iteration budget. Fits are now seeded per restart, convergence "
        "is determined from the iteration count instead of that flag, and a new "
        "Auto-calibrate button measures the dataset — iterations to convergence, "
        "restart agreement, observations per free parameter — and reports every "
        "criterion (AIC, AICc, BIC, ICL, and leave-one-session-out cross-validated "
        "likelihood) side by side so disagreement between them is visible. ICL is now "
        "the default, since AIC and BIC are known to over-select states on "
        "behavioural sequence data (Pohle et al. 2017).",
        "Permutation p-values can no longer be exactly zero. The transition- and "
        "occupancy-matrix tests divided the exceedance count by the number of "
        "permutations, which returns 0 whenever no shuffle beats the observed "
        "statistic — not a valid p-value, and one that survives FDR correction as an "
        "apparently infinitely significant cell. The estimator is now (b+1)/(m+1) "
        "(Phipson & Smyth 2010), so the floor is 1/(n_permutations+1) and is reported "
        "as such.",
        "Behavior presets replace the dead template dialog. Pick a built-in assay "
        "preset and Apply adds only the behaviors the project does not already "
        "define; Save as Preset stores the current project's definitions under the "
        "user's global config for reuse everywhere. Applying dedupes on name and "
        "never issues a duplicate keyboard shortcut, which would make the review "
        "soundboard ambiguous.",
        "Inference refuses to run on inputs that would produce a confidently wrong "
        "trace. Scoring a session whose target zone resolves to no area produced "
        "all-NaN ROI features; tree models route those rows down one default path and "
        "emit a near-constant trace, which reads downstream as a genuine absence of "
        "the behavior rather than an error. A second preflight audits the feature "
        "columns about to be scored, catching what the ROI config cannot see — a "
        "representation cache built before the zone was drawn, a partial R3D backfill, "
        "a column present under two spellings, a session extracted under different "
        "settings. Both are restricted to columns some model actually reads, and both "
        "name the offending sessions.",
        "Dense-inference R3D anchor embeddings are cached to disk. They are persisted "
        "under derived/r3d_features/dense_anchors/, keyed on the video and pose the "
        "crops came from plus the smoothing settings, so a second pass over the same "
        "session is a parquet read instead of GPU-hours. The cache sits deliberately "
        "outside derived/temporal_refinement/ so clearing the temporal cache cannot "
        "throw the embeddings away; the clear dialog now says so.",
        "New validation analysis: what labeling actually cost in human time, read "
        "from each project's review decision log rather than assumed. A clip's review "
        "time is the gap to the previous decision; gaps below the bulk-action "
        "threshold are one UI action and gaps above the break threshold are the "
        "reviewer stepping away, so neither is charged to a clip. Reported hours are "
        "therefore a floor, with an adjusted total that adds post-break clips back at "
        "the project's own median rate. It ships with its own GUI tab, PDF section, "
        "Prism exports and findings — including the caveat that the measured pace "
        "contradicts the 4 s/clip constant the rare-discovery effort figures assume.",
        "The Validation tab reports the target class, not the macro average. "
        "metrics.json stores macro precision/recall/F1 — the behavior averaged with "
        "“not the behavior” — and a one-vs-rest holdout is ~85% not-target, so the "
        "macro number sat far above the truth and could not be reconciled with the "
        "TP/FP/FN counts printed beside it. Headline scores now come from the model's "
        "own saved held-out probabilities as target-class metrics, with macro kept in "
        "a tooltip. Refined counts are also withheld on the same rule that already "
        "blanked the refined scores: publishing 6/4/40 next to an empty refined F1 "
        "handed the reader the exact quantity the suppression rule exists to "
        "withhold, just in another notation. Every counts cell now carries its full "
        "2x2 breakdown and the rates it implies.",
        "The learning-curve knee ships with an interval. The knee is a discrete "
        "argmin read off a mean curve, so a bare dashed line at “300 clips” read as a "
        "measurement to the clip when the replicates may have spanned several "
        "schedule steps. Both the knee and the F1 ceiling now carry a 95% percentile "
        "bootstrap over seeds, drawn as a shaded band on the figure, written to "
        "optimal_clips_summary.csv and carried through Prism and the findings text "
        "(where the recommended-budget median gets its own bootstrap over behaviors). "
        "Curves re-rendered from saved CSVs reuse the stored interval and draw no "
        "band at all when there is none, rather than a falsely tight one.",
        "Video-feature value reporting applies a multiplicity correction. The claim "
        "is about a family of behaviors (“video features improved N of M”), so a raw "
        "p at 0.05 across ~45 behaviors buys a couple of expected false positives. "
        "Every behavior now carries a BH-adjusted q_value and a significant_bh flag "
        "for figures to colour by; the original per-behavior flag is left untouched "
        "so older runs keep their meaning.",
        "The LOSO figure and Prism export show pooled target-class F1 with a "
        "subject-level bootstrap CI, replacing per-fold mean ± SEM — which loso.py "
        "itself already flagged invalid as an error bar, since folds are neither "
        "equally sized nor independent observations of one quantity. PR-AUC is drawn "
        "without a whisker rather than borrowing F1's interval.",
        "Spatial heatmaps and density analysis plotted the wrong frames. The "
        "Analytics tab rebases bouts to each subject's analysis prechop so latency "
        "and time-bin metrics start at test onset, but pose arrays are never rebased "
        "— they stay indexed by raw video frame. Both spatial views looked bout "
        "positions up using the rebased frames, so every plotted coordinate came from "
        "1000-4160 frames (33-139 s on the project this was found on) before the "
        "behavior happened, smearing every behavior into the same generic "
        "arena-occupancy map. They also re-applied the prechop a second time, "
        "clamping starts and silently dropping bouts. Frames are now mapped back into "
        "video space before any pose lookup.",
        "Renaming a behavior reaches the analytics graphs. Two independent caches "
        "held the old name: the disk analytics cache stores the behavior name on "
        "every row but fingerprints only the data files, and a rename touches "
        "behavior_definitions.yaml alone — so stale rows were replayed verbatim. "
        "Separately, merged external projects were matched to host behaviors by name "
        "only, so a host rename orphaned them. Rows are now relabeled from "
        "behavior_id, and the tab re-reads behavior definitions on refresh.",
        "“Bouts Until Behavior” with “Scale by pre-behavior time” exports what it "
        "graphs. That combination divides each subject's pre-target count by that "
        "subject's own pre-target interval, giving one rate per subject; the Excel "
        "export routed it through the time-binned path, emitting raw unscaled per-bin "
        "counts plus a total column — neither the graphed rate nor the labelled unit. "
        "It now writes one value per subject per behavior. Bar, box and stacked "
        "styles likewise export one value per subject instead of bins, matching the "
        "CSV export and the chart.",
        "New export: per-session {SubjectID}ABELposition.csv for TRACY, with a "
        "selectable track point (any tracked body part, or the centroid) and "
        "session-scoped subject naming that matches the boutframes export. Timestamps "
        "are written at 6 decimals because 4 quantises 30.000 fps into 30.030 and "
        "trips TRACY's frame-rate cross-check. Boutframes exports now also include "
        "subjects from combined (merged) projects, which were previously dropped.",
        "Temporal Review gained a trace playhead that tracks the video preview, and "
        "per-behavior positive thresholds are read from the behavior's own setting "
        "rather than the global one.",
        "GUI audit pass: readable text and no clipped controls down to 1366x768. The "
        "#546E7A used for secondary and disabled text measured ~2.6:1 on the dark "
        "ground and is replaced throughout; disabled state is restated for every "
        "widget class that carries text; destructive buttons get a real red ground "
        "instead of red text on blue (as low as 1.8:1); matplotlib's navigation "
        "toolbar is themed, having rendered black-on-navy at 1.21:1. Fifteen "
        "top-level tabs needed ~1620 px at the old minimum tab width, so four of them "
        "fell off the bar and were reachable only by chevron — tabs now size to their "
        "labels. Crowded button and control rows wrap onto extra rows instead of "
        "squeezing their labels, and the splash screen scrolls rather than setting a "
        "771 px floor for the whole window.",
        "Documentation consolidated. README.md is the single current document; "
        "readme.txt is now a Windows quickstart that points at it, instead of a "
        "second copy that had drifted. The two Phase-1 design documents in docs/ are "
        "marked historical rather than read as current architecture. The Methods "
        "tab's reference list is pruned to sources ABEL's code actually implements, "
        "and its test now checks that every formula's named source resolves to a real "
        "importable attribute. Eight one-off development scripts have been removed "
        "from the repository.",
        "Fixed a test-suite deadlock. The raw-data confirmation gate called "
        "QMessageBox.exec from the validation GUI's run path with no check for "
        "whether anyone could answer it, so under an offscreen Qt platform — the "
        "headless test suite, the benchmark and validation drivers — it spun a nested "
        "event loop that never returned and “pytest tests/” hung instead of failing. "
        "It now detects a non-interactive platform and proceeds, logging which "
        "projects had unreachable raw data. The hang had been stopping collection "
        "partway and hiding three stale tests, which are also fixed.",
    ]),
