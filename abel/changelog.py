...
CHANGELOG: list[tuple[str, str, list[str]]] = [
    ...
    ("0.22.0", "September 20, 2026", [
        ...
        "track and reads the darker track as the same mouse frame after "
        "frame, so an exchange that happens while the animals are touching is "
        "caught where the position-based detector could only see a jump. Data "
        "Import gains 'Scan Swaps (appearance)', which checks every session "
        "against its saved corrections, lists the ones that disagree and "
        "applies them in a single step. A session whose animals are not "
        "visually distinct is reported as undecidable rather than treated as "
        "clean.",
        "The multi-animal identity dialog navigates. Previous and Next Session "
        "step through the import table and save the current names on the way, "
        "Prev and Next Swap jump to the detected exchange points, a swap can "
        "be marked at the current frame, and Auto-Detect Swaps from Appearance "
        "fills the correction list for the open session. Each detected event "
        "carries its time and the pair involved, and is flagged when the "
        "tracks overlap or the pose is too scattered to trust.",
        "Changing identities no longer orphans labels. Renaming an individual "
        "or adding a swap correction rewrites every segment id that animal "
        "owns, so reviewer labels and soundboard labels follow the animal they "
        "describe. Labels whose window straddles a correction frame are listed "
        "separately, because the window holds both animals and no single id is "
        "right for it. Renaming one animal no longer reports the other "
        "animal's labels as moved.",
        "An animal added to the cage part-way through a session reads as "
        "absent instead of as a frozen phantom. Pose cleaning used to "
        "back-fill the leading undetected frames with the animal's first "
        "detected pose, which fabricated social distance and contact for the "
        "animals that really were there. Gaps up to 30 frames are still "
        "filled; a longer absence is carried as a presence mask that "
        "suppresses scoring across the whole scoring window rather than only "
        "its stride, drops a window that is only partly present instead of "
        "keeping it as an all-NaN row, and is excluded from every model's "
        "feature set.",
        "Subject and session patterns can be built by highlighting an example "
        "filename instead of writing a regular expression. Selecting the "
        "characters that name the subject generates a pattern that "
        "generalizes to the rest of the import, previewed against every "
        "filename with a live count of the videos that would pair with a pose "
        "file. Tracker suffixes (.tracked.sleap, DLC_resnet50...) do not "
        "disturb it. Pairing also falls back to a longest-common-prefix "
        "match, and to the subject and session keys, when video and pose "
        "stems differ.",
        "Targeted Clip Mining gains an Auto Hunter tab: ready-made hunt "
        "definitions for a project where nothing has been reviewed yet and "
        "there is no exemplar to point at. A preset names only features the "
        "schema always produces and weights a robust z-score taken over the "
        "target project's own pool, so it carries no millimeter, pixel scale "
        "or arena size across projects. A breadth control widens the "
        "descriptive box, and choosing a hard filter overrides the preset's "
        "preference.",
        "The Behavior Soundboard opens from the Review tab, groups its "
        "buttons, and labels every subject, or just the selected one, No "
        "Behavior in one click. A mislabeled clip can be cleared: the "
        "decision, the per-animal label rows and the soundboard payload all "
        "go, the candidate stays in the queue as unreviewed, and neighboring "
        "clips keep their labels. Clearing works on the current clip or on a "
        "selection.",
        "Filtering the Review tab's candidate list no longer checks the "
        "filesystem once per candidate. An Active Learning run persists every "
        "window it ranked while cutting clips for only a few hundred, so a "
        "real project's list runs to hundreds of thousands of rows and a "
        "single filter pass cost about 40 s of blocking probes. Clips now "
        "resolve against one directory scan.",
        "A window nominated by several behaviors keeps every nomination. "
        "Batch runs generate candidates one behavior at a time and the queue "
        "holds one row per segment, so storing a single behavior id meant "
        "each behavior overwrote the last: a 15-model run left only the final "
        "behavior's queue visible and nine behaviors looked as though they "
        "had produced nothing. A batch run also no longer piles the previous "
        "run's queue onto its own.",
        "Review clips can be cut full frame (no crop), and can be held wide "
        "enough to keep all animals in view, with a per-animal overlay legend "
        "sized in proportion to the clip that does not widen past its budget "
        "for long track names. A full-frame clip keeps the video's aspect "
        "ratio and ignores dynamic centering.",
        "Only the sessions whose tracking was fixed need re-extracting. Pose "
        "Features gains Select Needing Extraction, which picks the sessions "
        "whose cached features are missing or predate the current schema, and "
        "the cache can now be invalidated one session at a time instead of "
        "wholesale. Re-cut review clips for re-processed sessions regenerates "
        "those sessions' clips afterwards, so the overlays match the "
        "corrected tracks.",
        "Background-subtraction sensitivity is a real per-project extraction "
        "setting (motion.bg_var_threshold in environment_rois.yaml) rather "
        "than a preview-only control. Context extraction reads it and the "
        "context-cache signature hashes that file, so changing it rebuilds "
        "context features; a project that never touches it keeps a "
        "byte-identical file and a valid cache.",
        "The Graphs tab's ethogram follows the Group-by selection. It used to "
        "draw every checked subject regardless of the facet controls, so "
        "pinning a factor to one level still showed sessions from other "
        "groups; the plot, its session checklist and both exports now share "
        "one roster. The tab also gains a per-behavior filter, a per-subject "
        "prechop editor, a color-bar toggle for the density maps and a "
        "normalized difference metric for group comparison.",
        "Dense temporal inference reads multi-animal sessions correctly. The "
        "frame table carries one row per (frame, animal), and treating those "
        "rows as the time axis halved every window's real duration in a "
        "two-animal session, blended both animals into every window, and "
        "stretched the probability trace over twice as many frames as the "
        "video has, leaving the back half of every trace flat at zero.",
        "The Temporal Review trace panel gives its spare height to the plot "
        "instead of the control rows, uses a palette that scales past eight "
        "behaviors, draws the legend beside the plot rather than on top of "
        "it, and can show one animal's trace or the maximum across animals.",
        "Active Learning gains a session chooser for candidate generation, a "
        "Generate Review Clips button, a cross-behavior confound view and a "
        "unified UMAP over all behaviors, and its visualizations can be saved "
        "to file. Apply Models gains a behavior map for imported models.",
        "A pre-release audit fixed a crash in clip writing (a variable read "
        "before it was assigned), label-map handling that could disagree with "
        "itself about which class is the behavior, and a social-distance "
        "column whose _norm suffix doubled on every rebuild and so kept 80 "
        "body-length-normalized features out of training.",
        "Interface text, tooltips and documentation use American spelling "
        "throughout and no longer contain em dashes, which some Windows fonts "
        "and spreadsheet importers rendered as mojibake.",
    ]),
    ("0.21.0", "September 12, 2026", [
        ...
        "windows made MOG2 re-initialize and read the next frame as 100% "
        ...
        "for black pixels blanked every icon; it now recolors by alpha.",
    ...
    ("0.14.0", "September 3, 2026", [
        ...
        "the problem. Three defenses: traces are decimated at capture with a 1 px "
        ...
        "reads back. An unnamed subject is labeled by its session id once rather "
    ...
    ("0.13.0", "August 31, 2026", [
        ...
        "matches the current data is labeled STALE rather than silently presented as "
        ...
        "at any zoom, then repelled from its neighbors on a leash that stops it "
    ...
    ("0.12.0", "August 27, 2026", [
        ...
        "behavioral sequence data (Pohle et al. 2017).",
        ...
        "for figures to color by; the original per-behavior flag is left untouched "
        ...
        "counts plus a total column: neither the graphed rate nor the labeled unit. "
        ...
        "are written at 6 decimals because 4 quantizes 30.000 fps into 30.030 and "
    ...
    ("0.11.0", "August 16, 2026", [
        ...
        "Distances are now canonicalized both when the training set is written and when "
        ...
        "thresholds. Given exemplar clips, an L1-penalized logistic fit selects a sparse "
    ...
    ("0.10.0", "July 23, 2026", [
        ...
        "feature family removes, colored by which modality does the work, so the "
        ...
        "instead of rendering in the same gray as “already solved by the baseline”, a "
    ...
    ("0.9.0", "July 21, 2026", [
        ...
        "neighborhoods) actually finds a rare behavior faster than random or "
    ...
    ("0.8.0", "July 16, 2026", [
        ...
        "to a zone, centroid speed, distance traveled, body elongation, …), then "
