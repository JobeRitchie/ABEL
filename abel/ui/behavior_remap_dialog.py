"""Behavior name remapping helper for the Model Refinement tab.

Different projects sometimes give the same behavior different names, one
labels it "Dip", another "Head Dip".  Because cross-project import matches
behaviors by name, those examples would be silently skipped.  This dialog lets
the user map each unmatched source behavior onto an existing host behavior,
...
# Sentinel combo entry meaning "leave unmapped / don't import this behavior".
_SKIP = "(skip, do not import)"


class BehaviorRemapDialog(QDialog):
    """Map differently named source behaviors onto host behaviors.
    ...
    rows:
        ...
        behavior that does not already match a host behavior by exact name.
        ``suggested_host_name`` is "" when no good guess exists.
    host_names:
        All host behavior names available as remap targets.
    current_aliases:
    ...
    def __init__(
        ...
        self.setWindowTitle(f"Remap Behavior Names: {source_tag}")
        self.resize(620, 420)
        ...
        intro = QLabel(
            "These behaviors from <b>{tag}</b> have no behavior of the same "
            "name in this project, so their examples are skipped on import. "
            "Map each one to the host behavior it really is (e.g. "
        ...
        self._table.setHorizontalHeaderLabels(
            ["Source behavior", "Examples", "Map to host behavior"]
        )
    ...
    def aliases(self) -> dict[str, str]:
        ...
        Source behaviors left on *skip* are omitted.
        """
        out: dict[str, str] = {}
